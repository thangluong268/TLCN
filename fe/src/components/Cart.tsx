...
function Cart(props: PropsCartInterface) {
    ...
    const deleteProductInCartLocal = async (): Promise<void> => {
        ...
        const shouldDelete = window.confirm("Bạn có muốn xóa sản phẩm này khỏi giỏ hàng không?")
        ...
        cart.map((store: CartInterface) => {
            store.listProducts = store.listProducts.filter((product: ProductBillDto) => product.productId !== prop.productId)
            return store
        })
    ...
    const isChecked = (e: any) => {
        const lengthListCart = document.querySelector(`.store-parent-${idStore}`)?.querySelectorAll('[class*="checkbox-item-"]').length;
        let checkedItems = 0;
        document.querySelector(`.store-parent-${idStore}`)?.querySelectorAll('[class*="checkbox-item-"]').forEach((item: any) => {
            if (item.checked) {
                checkedItems++;
            }
        });
        const store = document.querySelector(`.checkbox-store-${idStore}`) as HTMLInputElement;
        const all = document.querySelector(`.checkbox-cart-all`) as HTMLInputElement;
        if (lengthListCart === checkedItems) {
            store.checked = true;
            //
            const lengthListCart1 = document.querySelectorAll('[class*="store-parent-"]').length;
            let checkedItems = 0;
    ...
    return (
        <div className={`flex justify-between items-center p-2 rounded-lg text-center `}>
            ...
            <div className="flex items-center justify-center">
                <button className="px-2 py-1 border border-[#b6caf2] rounded-l-lg" onClick={(e) => handleDecrease()}>
                    -
                </button>
            ...
            <span className="text-[14px] w-[20%]">{FormatMoney(prop.price * quantity)}</span>
            ...
            <div className='flex flex-col items-center w-[15%]'>
                ...
                <span className='text-[14px] max-w-[70%] text-[#648fe3] cursor-pointer'>Tìm sản phẩm tương tự</span>
            </div>
        </div>
