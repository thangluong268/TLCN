...
function Header() {
  ...
  React.useEffect(() => {
    ...
    setCountNewNoti(
      dataNoti.notifications.filter((item) => item.status === false).length
    );
  }, [dataCart]);

  return (
    <header className="h-[60px]">
      <div className="flex justify-between items-center w-full h-[60px] bg-[#D2E0FB] px-[160px] fixed top-0 left-0 right-0 z-10">
        <img
